# Critial Instructions!
 ...
 - When you need to search docs, use `context7` tools. Always search docs before writing any code.
 - When working with UI components, use `shadcn` or `magicui` tools. Always prefer premade components over coming up with them from scratch.
 - When working on UI or making large changes, use the `playwright` tools to navigate in the browser.
 - Whenever needing broader domain research, use the `brave-search` tool to search the web.
